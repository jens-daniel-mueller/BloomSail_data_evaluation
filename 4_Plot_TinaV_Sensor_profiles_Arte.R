...
Sensor_profile_long_mean %>% 
  filter(parameter %in% c("Temperature (°C)", "CT (µmol/kg)")) %>% 
  ggplot()+
...
Sensor_profile_long_mean %>% 
  filter(parameter %in% c("Temperature (°C)", "CT (µmol/kg)"),
         ID %in% c("180705", "180718", "180802", "180820")) %>% 
  ggplot()+
  geom_path(aes(value, dep_int, col=ID, group=interaction(ID, station)))+
  scale_y_reverse()+
  facet_wrap(~parameter, scales = "free_x", ncol = 1)+
  scale_color_viridis_d(name="Datum")+
  labs(y="Depth (m)", x="")+
  theme_bw()

ggsave(here::here("/Plots/TinaV/Sensor", "Sensor_profiles_by_parameters_mean_Arte.jpg"), width = 5, height = 6, dpi=300)
